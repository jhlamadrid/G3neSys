...
$old_error_handler = set_error_handler("ctrl_login_scriptcase_error_handler");

function ctrl_login_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['ctrl_login']['contr_erro']) && $_SESSION['scriptcase']['ctrl_login']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_ctrl_login->contr_ctrl_login->Apl_com_erro = true;
            if ($inicial_ctrl_login->contr_ctrl_login->NM_ajax_flag)
            {
                $inicial_ctrl_login->contr_ctrl_login->NM_ajax_info['errList']['geral_ctrl_login'][] = $msg;
                return;
            }
            ctrl_login_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function ctrl_login_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['ctrl_login'])  && '' != $_SESSION['scriptcase']['error_icon']['ctrl_login'])  ? $_SESSION['scriptcase']['error_icon']['ctrl_login']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['ctrl_login']) && '' != $_SESSION['scriptcase']['error_close']['ctrl_login']) ? $_SESSION['scriptcase']['error_close']['ctrl_login'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['ctrl_login']) && '' != $_SESSION['scriptcase']['error_span_title']['ctrl_login']) ? $_SESSION['scriptcase']['error_span_title']['ctrl_login'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['ctrl_login']) && '' != $_SESSION['scriptcase']['error_icon_title']['ctrl_login']) ? $_SESSION['scriptcase']['error_icon_title']['ctrl_login'] : '';
?>
