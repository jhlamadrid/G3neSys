...
class Oordpag_model extends CI_MODEL {
    ...
    function get_all_oordpag($inicio, $fin){
    	...
    	$query = $this->oracle->query("SELECT * 
                              FROM TESTPRDGESTIONCOMERCIAL.OORDPAG 
                              WHERE    
                                    CTDCOD = 3 AND 
                                    DOCCOD = 4 AND
                                    SORCOD = 1 AND
    ...
    function get_oordserv($EMPCOD, $OFICOD, $ARECOD, $CTDCOD, $DOCCOD, $SERNRO, $ORPNRO){
        $query = $this->oracle->query("SELECT * 
                                        FROM TESTPRDGESTIONCOMERCIAL.OCENDOC
                                        WHERE   STPEMPCOD = ? AND 
                                                STPOFICOD = ? AND 
                                                STPARECOD = ? AND 
                                                STPCTDCOD = ? AND 
    ...
    function get_concep_oordpag($EMPCOD, $OFICOD, $ARECOD, $CTDCOD, $DOCCOD, $SERNRO, $ORPNRO){
        $query = $this->oracle->query("SELECT 
                                            TESTPRDGESTIONCOMERCIAL.OORDPAG1.ORPCNT,
                                            TESTPRDGESTIONCOMERCIAL.OORDPAG1.STOPCUMCOD,
                                            TESTPRDGESTIONCOMERCIAL.OORDPAG1.ORPPRE,
                                            PRDDBFCOMERCIAL.CONCEP.*
                                        FROM TESTPRDGESTIONCOMERCIAL.OORDPAG1
                                        INNER JOIN PRDDBFCOMERCIAL.CONCEP
                                        ON PRDDBFCOMERCIAL.CONCEP.FACCONCOD = TESTPRDGESTIONCOMERCIAL.OORDPAG1.CPTCOD
                                        WHERE   EMPCOD = ? AND 
                                                OFICOD = ? AND
                                                ARECOD = ? AND 
                                                CTDCOD = ? AND 
    ...
    function existe_usuario($oordserv){
        $query = $this->oracle->query("select 
                                        ...
                                        X.DIRECCION
                                        FROM TESTPRDGESTIONCOMERCIAL.OSOLACC
                                        ...
                                        FROM TESTPRDGESTIONCOMERCIAL.MCLISOL
                                        INNER JOIN TESTPRDGESTIONCOMERCIAL.CUBIGEO ON MCLISOL.CDPCOD = CUBIGEO.CDPCOD 
                                        INNER JOIN TESTPRDGESTIONCOMERCIAL.CUBIGEO1 ON MCLISOL.CDPCOD = CUBIGEO1.CDPCOD AND MCLISOL.CPVCOD = CUBIGEO1.CPVCOD 
                                        INNER JOIN TESTPRDGESTIONCOMERCIAL.CUBIGEO2 ON MCLISOL.CDPCOD = CUBIGEO2.CDPCOD AND MCLISOL.CPVCOD = CUBIGEO2.CPVCOD AND MCLISOL.CDSCOD = CUBIGEO2.CDSCOD
                                        INNER JOIN TESTPRDGESTIONCOMERCIAL.CUBIGEO3 ON MCLISOL.CDPCOD = CUBIGEO3.CDPCOD AND MCLISOL.CPVCOD = CUBIGEO3.CPVCOD AND MCLISOL.CDSCOD = CUBIGEO3.CDSCOD AND MCLISOL.CGPCOD = CUBIGEO3.CGPCOD
                                        INNER JOIN TESTPRDGESTIONCOMERCIAL.MVIAS ON MCLISOL.MVICOD = MVIAS.MVICOD ) X
